...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
                              ...
                              num_workers=1, pin_memory=False, **kwargs): # train_loader & test_loader
    ...
    # RandomSampler
    train_sampler = RandomSampler(indices[:train_size])
    val_sampler = RandomSampler(
        indices[-(valid_size + test_size):-test_size])
    if return_test:
        # RandomSampler
        test_sampler = RandomSampler(indices[-test_size:])
...
def calculateDistance(a,b):   # Atom-wise OFM
    """
    Calculate the Euclidea distance between the atoms.
    """
...
def make_hot_for_atom_i(crystal,i,hvs):
    """
    Extract the OFM features.

    Parameters
    ----------
    crytal: the cif structure of the crystals

    Returns:
    OFM matrix: 32x32 sized matrix 
    """
...
#        r_pk1 = calculateDistance(nb['site'].coords,crystal[i].coords)
#        r_pk1 = calculateDistance(nb['site'].coords,crystal[i].coords)
        r_pk = (calculateDistance(nb['site'].coords,crystal[i].coords))*(calculateDistance(nb['site'].coords,crystal[i].coords))
#         for i in range(4):
            
#             r_pk =  (calculateDistance(nb['site'].coords,crystal[i].coords))*r_pk
#         for i in range(8):
#             r_pk2 = r_pk2 * calculateDistance(nb['site'].coords,crystal[i].coords)
        HV_K = hvs[EK]
        HV_K = HV_K.reshape((HV_K.shape[1], 32))
        coef_K = (angle_K/max_angle)/(r_pk)
#        coef_K = (angle_K/max_angle)*(1/r_pk - 1/r_pk2)
...
class GaussianDistance(object):   # 12 nbrs' Gaussian distance the length of the vector is 41
    """
...
class AtomInitializer(object):    #Atom initializer
	"""
...
class AtomCustomJSONInitializer(AtomInitializer):  # JSON initializer
	"""
...
class CIFData(Dataset):    # process the CIF files
	"""
	The CIFData dataset is a wrapper for a dataset where the crystal structures
	are stored in the form of CIF files. The dataset should have the following directory structure:
    ...
    id_prop.csv: a CSV file with two columns. The first column records a unique ID 
    for each crystal, and the second column recods the value of the target property.

    atom_init.json: a JSON file that stored the initialization vector for each element.

    ID.cif: a CIF file that recodes the crystal structure, where ID is the unique ID
    for the crystal.
    ...
    random_seed: int
    	Random seed for shufflinf the dataset
    ...
    def __init__(self, root_dir, max_num_nbr=12, radius=8, dmin=0, step=0.2,
        ...
        with open(id_prop_file) as f:
            reader = csv.reader(f)
            self.id_prop_data = [row for row in reader]    ## The data which in the csv file
    ...
    def __getitem__(self, idx):
        cif_id, target = self.id_prop_data[idx]      #get the cif file id and the target values accordingly 
        crystal = Structure.from_file(os.path.join(self.root_dir,
                                                   cif_id+'.cif'))     # read the cif file with the according id 
        atom_fea = np.vstack([self.ari.get_atom_fea(crystal[i].specie.number)
                              for i in range(len(crystal))])        # the atom feature 
        atom_fea = torch.Tensor(atom_fea)                           # transfer the atom feature to the tensor 
        ## Adding for OFM
        elements = {'H':['1s2'],'Li':['[He] 1s2'],'Be':['[He] 2s2'],'B':['[He] 2s2 2p1'],'N':['[He] 2s2 2p3'],'O':['[He] 2s2 2p4'],
                     ...
                     'Hs':['[Rn] 5f14 6d6 7s2'],'Mt':['[Rn] 5f14 6d7 7s2'],'Xe': ['[Kr] 4d10 5s2 5p6'], 'He':['1s2']}
        ...
        all_nbrs = crystal.get_all_neighbors(self.radius, include_index=True)    # neighbors 
